...
export default {
  ...
  execute: async ({ sock, msg, args, phoneNumber, isAdmin }) => {
    ...
    if (!targetJid) {
      await sock.sendMessage(jid, { text: 'Please mention or reply to the member you want to unmute, or provide their number.' }, { quoted: msg });
      return;
    }
    ...
    if (!settings.mutedMembers || !settings.mutedMembers.includes(targetJid)) {
      await sock.sendMessage(jid, { text: `@${targetJid.split('@')[0]} is not currently muted.` }, { quoted: msg });
      return;
    }

    settings.mutedMembers = settings.mutedMembers.filter(member => member !== targetJid);
    updateChatSettings(phoneNumber, jid, { mutedMembers: settings.mutedMembers });

    await sock.sendMessage(jid, {
      text: `@${targetJid.split('@')[0]} has been unmuted.`, 
      mentions: [targetJid]
    }, { quoted: msg });
    logger.info(`[UNMUTEMEMBER] ${targetJid} unmuted in ${jid} by ${sender}`);
  }
};
